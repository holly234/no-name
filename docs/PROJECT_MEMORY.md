...
- Provider-neutral AI and credit enforcement pass on 2026-07-20:
  ...
  - Full verification passes with `117 tests, 519 assertions`; production Vite build and Blade view compilation also pass.
...
- Added the first dedicated channel product page at `/products/instagram` (`marketing.instagram`): Instagram-specific hero artwork, comment/DM/follower use cases, a conversion workflow section, and an Instagram-focused FAQ. Product navigation links to it directly; future channel pages should follow this dedicated-page pattern rather than cloning the homepage.
- Product-page layout rule: use the same wide editorial rhythm as the main marketing page, with generous outer margins and breathing room. Avoid narrow dashboard-like panels, dense card grids, thick borders, and decorative AI/chat mockups unless explicitly requested.
